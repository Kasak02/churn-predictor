...
app = FastAPI(
    ...
    """,
    version="1.0.0",
    contact={"name": "Kasak", "url": "https://github.com/Kasak02/churn-predictor"},
)
...
def check_models_loaded():
    ...
    if model is None:
        raise HTTPException(
            status_code=503, detail="Models not loaded. Run from project root."
        )
...
class CustomerData(BaseModel):
    ...
    # Core features
    tenure: float = Field(..., ge=0, le=72, description="Months as customer (0-72)")
    MonthlyCharges: float = Field(..., ge=0, description="Monthly bill in USD")
    TotalCharges: float = Field(0.0, ge=0, description="Total charges paid")
    ...
    SeniorCitizen: int = Field(0, ge=0, le=1, description="0=No, 1=Yes")
...
def predict_batch(customers: list[CustomerData]):
    ...
    if len(customers) > 1000:
        raise HTTPException(
            status_code=400, detail="Maximum 1000 customers per batch request"
        )
...
def explain(customer: CustomerData):
    ...
    try:
        ...
        top_features = [
            {
                ...
                "direction": (
                    "increases churn risk" if val > 0 else "decreases churn risk"
                ),
                "impact": (
                    "HIGH" if abs(val) > 0.2 else "MEDIUM" if abs(val) > 0.1 else "LOW"
                ),
            }
        ...
        top_positive = [
            f["feature"].replace("_", " ") for f in top_features if f["shap_value"] > 0
        ][:2]
        ...
        summary = (
            ...
            if top_positive
            else f"This customer has a {prob*100:.1f}% churn probability "
